...
class _AsyncWorker(QObject):
    ...
    def run(self) -> None:
        ...
        async def _main() -> None:
            ...
            # Signal Qt that the server is up
            self.ready.emit()
            logger.info("ui.app | server pronto → {}:{}", host, port)
            ...
            try:
                ...
                logger.warning("ui.app | persist callback: {}", exc)

            # Ripristina sessioni salvate da disco
            try:
                saved = _load_sessions_disk()
                if saved and isinstance(saved, dict) and len(saved) > 0:
                    # Sostituisci completamente le sessioni di default
                    # con quelle salvate (con i loro messaggi)
                    ui_loop._sessions = saved
                    # Punta alla sessione usata più di recente
                    most_recent = max(
                        saved.items(),
                    ...
                    logger.info(
                        "ui.app | {} sessioni ripristinate ({} messaggi)",
                        len(saved), n_msgs,
                    )
            except Exception as exc:
                logger.warning("ui.app | restore sessions: {}", exc)

            state["loop"] = ui_loop

            async def _restore_ui_settings() -> None:
                """
                Ripristina modello/voce/personalità da ui-settings.json.
                Eseguita DOPO ui_loop.load() (quindi con _orch e _tts pronti),
                altrimenti switch_personality/switch_voice fallirebbero perché
                i moduli sottostanti non sono ancora inizializzati.
                Errori non fatali: logga e prosegue col default.
                """
                try:
                    from ui.server import _load_ui_settings
                    s = _load_ui_settings() or {}
                except Exception as exc:
                    logger.warning("ui.app | restore ui-settings (load): {}", exc)
                    return
                ...
                if saved_personality:
                    try:
                        ...
                        ok = False
                        logger.warning(
                            "ui.app | switch_personality '{}': {}",
                            saved_personality, exc,
                        )
                    logger.info(
                        "ui.app | personalità ripristinata → '{}'"
                        if ok else
                        "ui.app | personalità salvata non valida: '{}' — uso default",
                        saved_personality,
                    )
                ...
                if saved_model:
                    try:
                        ...
                        ok = False
                        logger.warning(
                            "ui.app | switch_model '{}': {}", saved_model, exc
                        )
                    logger.info(
                        "ui.app | modello ripristinato → '{}'"
                        if ok else
                        "ui.app | modello salvato non valido: '{}' — uso default",
                        saved_model,
                    )
                ...
                if saved_voice:
                    try:
                        ...
                        ok = False
                        logger.warning(
                            "ui.app | switch_voice '{}': {}", saved_voice, exc
                        )
                    logger.info(
                        "ui.app | voce ripristinata → '{}'"
                        if ok else
                        "ui.app | voce salvata non valida: '{}' — uso default",
                        saved_voice,
                    )
